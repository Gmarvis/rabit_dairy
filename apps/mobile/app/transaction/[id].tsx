    ...
    return (
      <View style={styles.screen}>
        <View style={{ paddingHorizontal: space(4) }}>
          ...
          <Text style={styles.dim}>{isLoading ? "Loading…" : "This transaction no longer exists."}</Text>
        </View>
      </View>
    );
  }
    ...
    <View style={styles.screen}>
      <View style={{ paddingHorizontal: space(4) }}>
        ...
        {txn.voiceTranscript ? (
          <Card style={styles.transcriptCard}>
            <Text style={styles.transcript}>&ldquo;{txn.voiceTranscript}&rdquo;</Text>
          </Card>
        ) : null}
      ...
      <View style={[styles.bottom, { paddingBottom: insets.bottom + space(2) }]}>
        ...
        <Pressable onPress={confirmDelete} disabled={remove.isPending} hitSlop={8}>
          <Text style={styles.delete}>{remove.isPending ? "Deleting…" : "Delete transaction"}</Text>
        </Pressable>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  dim: { color: colors.ink2, fontSize: 13, marginTop: space(4) },
  ...
  note: { backgroundColor: colors.card, borderColor: colors.line, borderWidth: 1, borderRadius: radius.md, paddingHorizontal: space(3), paddingVertical: space(2.5), color: colors.ink, fontSize: 13 },
  transcriptCard: { marginTop: space(3), backgroundColor: "rgba(233,180,76,0.08)", borderColor: "rgba(233,180,76,0.3)" },
  transcript: { color: colors.ink, fontSize: 13, fontStyle: "italic", lineHeight: 19 },
  ...
  delete: { color: colors.negative, fontSize: 13, fontWeight: "700", textAlign: "center" },
});
