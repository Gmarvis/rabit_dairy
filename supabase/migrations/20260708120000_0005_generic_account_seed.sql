-- Refactor the signup seed so new users start neutral: keep the generic
-- starter categories, but replace the previous personal account set (specific
-- banks, mobile-money and a dormant account) with a single "Cash" wallet.
-- Everyone has cash, so a new user can log immediately, then adds their own
-- bank / mobile-money accounts in the app.
...
begin
  ...
  insert into accounts (user_id, name, type, institution, mask, is_primary, is_dormant) values
    (uid, 'Cash', 'cash', null, null, true, false);
end;
$$;
